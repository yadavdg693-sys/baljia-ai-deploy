> Adapted from [nexu-io/open-design](https://github.com/nexu-io/open-design) `craft/accessibility-baseline.md` (Apache License 2.0).
> Upstream: https://github.com/nexu-io/open-design/blob/2afb002a6285f92ec80e6cee97f867dc7a680a77/craft/accessibility-baseline.md
...
commitments that go beyond it. The skeleton's `app/globals.css`
shadcn/ui CSS variables (`--primary`, `--accent`, `--ring`,
`--background`, `--foreground`, `--muted`, `--border`) consumed via
Tailwind classes decide brand appearance; this file decides which rules
an artifact has to clear before it ships.
...
checked items live in sibling craft files
...
In the skeleton, use `ring-ring` (`--ring`) via Tailwind's `focus-visible:ring-2 focus-visible:ring-ring` pattern
on interactive elements to satisfy focus visibility requirements.
...
structural conventions treated as craft commitments. WCAG levels
are noted per item.
...
- **Heading hierarchy** (1.3.1 Info and Relationships Level A; 2.4.6 Headings and Labels Level AA): WCAG requires programmatically-determined structure and descriptive headings, not a specific outline shape. Craft convention layers on: prefer one `<h1>` per page and don't skip levels (`<h1>` → `<h3>` without `<h2>`). Visual size and heading level are independent.
...
times within any one-second period is non-conformant unless the flash area
...
## Common mistakes (guidance for the agent and the verifier; not auto-enforced today)
...
- Using `aria-description` as the sole state-carrier on `role="row"`. JAWS 2025/2026 silently drops it.
