...
export interface GrowMarketResearch {
  ...
  main_growth_bottleneck: string;
  customer_wedge: string;
  offer_packaging_direction: string;
...
export type MarketResearchResult = BuildMarketResearch | GrowMarketResearch;

// 3-section mission (replaces 1-line output)
export interface MissionDoc {
  one_liner: string;              // ≤14 words / ~80 chars, dashboard topbar
  mission: string;                // 1 sentence (aspirational)
...
export interface PipelineContext {
  ...
  landingPageBrief?: LandingPageBrief;
